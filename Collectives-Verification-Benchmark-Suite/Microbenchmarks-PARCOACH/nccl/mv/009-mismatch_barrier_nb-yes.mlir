module attributes {dlti.dl_spec = #dlti.dl_spec<#dlti.dl_entry<i16, dense<16> : vector<2xi32>>, #dlti.dl_entry<i8, dense<8> : vector<2xi32>>, #dlti.dl_entry<i1, dense<8> : vector<2xi32>>, #dlti.dl_entry<!llvm.ptr, dense<64> : vector<4xi32>>, #dlti.dl_entry<f128, dense<128> : vector<2xi32>>, #dlti.dl_entry<f64, dense<64> : vector<2xi32>>, #dlti.dl_entry<f16, dense<16> : vector<2xi32>>, #dlti.dl_entry<i32, dense<32> : vector<2xi32>>, #dlti.dl_entry<!llvm.ptr<272>, dense<64> : vector<4xi32>>, #dlti.dl_entry<!llvm.ptr<271>, dense<32> : vector<4xi32>>, #dlti.dl_entry<!llvm.ptr<270>, dense<32> : vector<4xi32>>, #dlti.dl_entry<f80, dense<128> : vector<2xi32>>, #dlti.dl_entry<i64, dense<64> : vector<2xi32>>, #dlti.dl_entry<"dlti.endianness", "little">, #dlti.dl_entry<"dlti.stack_alignment", 128 : i32>>, llvm.data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128", llvm.target_triple = "x86_64-unknown-linux-gnu", "polygeist.target-cpu" = "x86-64", "polygeist.target-features" = "+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87", "polygeist.tune-cpu" = "generic", spmd.executionKind = "All"} {
  func.func @main(%arg0: i32, %arg1: memref<?xmemref<?xi8>>) -> i32 attributes {llvm.linkage = #llvm.linkage<external>, spmd.executionKind = "All"} {
    ...
    %alloca_0 = memref.alloca() {spmd.executionKind = "All"} : memref<1x!llvm.struct<(array<128 x i8>)>>
    %alloca_1 = memref.alloca() {spmd.executionKind = "All"} : memref<1xmemref<?x!llvm.struct<()>>>
    %3 = "spmd.castStream"(%alloca_1) <{usedModel = 1 : i32}> {spmd.executionKind = "All"} : (memref<1xmemref<?x!llvm.struct<()>>>) -> !spmd.stream
    %cast = memref.cast %alloca_1 {spmd.executionKind = "All"} : memref<1xmemref<?x!llvm.struct<()>>> to memref<?xmemref<?x!llvm.struct<()>>>
    ...
    %cast_4 = memref.cast %alloca_3 {spmd.executionKind = "All"} : memref<1x!llvm.struct<(array<128 x i8>)>> to memref<?x!llvm.struct<(array<128 x i8>)>>
    ...
    scf.if %6 {
      %25 = func.call @ncclGetUniqueId(%cast_4) {spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static"} : (memref<?x!llvm.struct<(array<128 x i8>)>>) -> i32
    } {spmd.executionKind = "All", spmd.isMultiValued = true}
    %7 = "polygeist.memref2pointer"(%alloca_3) {spmd.executionKind = "All"} : (memref<1x!llvm.struct<(array<128 x i8>)>>) -> !llvm.ptr
    %8 = "polygeist.pointer2memref"(%7) {spmd.executionKind = "All"} : (!llvm.ptr) -> memref<?xi8>
    %error_5 = "spmd.bcast"(%2, %8, %8, %c128_i64, %1, %c0_i32) <{isBlocking = true, usedModel = 0 : i32}> {spmd.executionKind = "All"} : (!spmd.comm, memref<?xi8>, memref<?xi8>, i64, !spmd.datatype, i32) -> !spmd.error
    %9 = call @cudaSetDevice(%c0_i32) {spmd.executionKind = "All"} : (i32) -> i32
    %10 = "polygeist.memref2pointer"(%alloca_2) {spmd.executionKind = "All"} : (memref<1xmemref<?xi32>>) -> !llvm.ptr
    ...
    %14 = "polygeist.memref2pointer"(%alloca_0) {spmd.executionKind = "All"} : (memref<1x!llvm.struct<(array<128 x i8>)>>) -> !llvm.ptr
    scf.for %arg2 = %c0 to %c128 step %c1 {
      %25 = arith.index_cast %arg2 {spmd.executionKind = "All"} : index to i32
      %26 = llvm.getelementptr %7[%25] {spmd.executionKind = "All"} : (!llvm.ptr, i32) -> !llvm.ptr, i8
      %27 = llvm.load %26 {spmd.executionKind = "All"} : !llvm.ptr -> i8
      %28 = llvm.getelementptr %14[%25] {spmd.executionKind = "All"} : (!llvm.ptr, i32) -> !llvm.ptr, i8
    ...
    %rank_6, %error_7 = "spmd.getRankInComm"(%4) <{usedModel = 1 : i32}> {spmd.executionKind = "All"} : (!spmd.comm) -> (i32, !spmd.error)
    %16 = arith.remsi %rank_6, %c2_i32 {spmd.executionKind = "All"} : i32
    %17 = arith.cmpi ne, %16, %c0_i32 {spmd.executionKind = "All"} : i32
    scf.if %17 {
      %25 = memref.load %alloca_2[%c0] {spmd.executionKind = "Dynamic"} : memref<1xmemref<?xi32>>
      %26 = "polygeist.memref2pointer"(%25) {spmd.executionKind = "Dynamic"} : (memref<?xi32>) -> !llvm.ptr
      %27 = "polygeist.pointer2memref"(%26) {spmd.executionKind = "Dynamic"} : (!llvm.ptr) -> memref<?xi8>
      %error_8 = "spmd.allgather"(%4, %27, %c0_i64, %0, %27, %c0_i64, %0, %3) <{isBlocking = true, usedModel = 1 : i32}> {spmd.executionKind = "Dynamic"} : (!spmd.comm, memref<?xi8>, i64, !spmd.datatype, memref<?xi8>, i64, !spmd.datatype, !spmd.stream) -> !spmd.error
    } else {
      %25 = memref.load %alloca_2[%c0] {spmd.executionKind = "Dynamic"} : memref<1xmemref<?xi32>>
      %26 = "polygeist.memref2pointer"(%25) {spmd.executionKind = "Dynamic"} : (memref<?xi32>) -> !llvm.ptr
      %27 = "polygeist.pointer2memref"(%26) {spmd.executionKind = "Dynamic"} : (!llvm.ptr) -> memref<?xi8>
      %req, %error_8 = "spmd.allgather"(%4, %27, %c0_i64, %0, %27, %c0_i64, %0, %3) <{isBlocking = false, usedModel = 1 : i32}> {spmd.executionKind = "Dynamic"} : (!spmd.comm, memref<?xi8>, i64, !spmd.datatype, memref<?xi8>, i64, !spmd.datatype, !spmd.stream) -> (!spmd.req, !spmd.error)
      memref.store %req, %alloca[%c0] {spmd.executionKind = "Dynamic"} : memref<1x!spmd.req>
      %alloca_9 = memref.alloca() {spmd.executionKind = "Dynamic"} : memref<1x!spmd.status>
      %28 = "spmd.waitAll"(%c1_i32, %alloca, %alloca_9) <{usedModel = 1 : i32}> {spmd.executionKind = "Dynamic"} : (i32, memref<1x!spmd.req>, memref<1x!spmd.status>) -> !spmd.error
    } {spmd.executionKind = "All", spmd.isMultiValued = true}
    %18 = memref.load %alloca_1[%c0] {spmd.executionKind = "All"} : memref<1xmemref<?x!llvm.struct<()>>>
    %19 = call @cudaStreamSynchronize(%18) {spmd.executionKind = "All"} : (memref<?x!llvm.struct<()>>) -> i32
    %20 = memref.load %alloca_2[%c0] {spmd.executionKind = "All"} : memref<1xmemref<?xi32>>
    ...
    %23 = call @cudaFree(%22) {spmd.executionKind = "All"} : (memref<?xi8>) -> i32
