module attributes {dlti.dl_spec = #dlti.dl_spec<#dlti.dl_entry<i64, dense<64> : vector<2xi32>>, #dlti.dl_entry<f80, dense<128> : vector<2xi32>>, #dlti.dl_entry<!llvm.ptr<270>, dense<32> : vector<4xi32>>, #dlti.dl_entry<f128, dense<128> : vector<2xi32>>, #dlti.dl_entry<!llvm.ptr<272>, dense<64> : vector<4xi32>>, #dlti.dl_entry<!llvm.ptr<271>, dense<32> : vector<4xi32>>, #dlti.dl_entry<i32, dense<32> : vector<2xi32>>, #dlti.dl_entry<i16, dense<16> : vector<2xi32>>, #dlti.dl_entry<f64, dense<64> : vector<2xi32>>, #dlti.dl_entry<f16, dense<16> : vector<2xi32>>, #dlti.dl_entry<!llvm.ptr, dense<64> : vector<4xi32>>, #dlti.dl_entry<i8, dense<8> : vector<2xi32>>, #dlti.dl_entry<i1, dense<8> : vector<2xi32>>, #dlti.dl_entry<"dlti.stack_alignment", 128 : i32>, #dlti.dl_entry<"dlti.endianness", "little">>, llvm.data_layout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-f80:128-n8:16:32:64-S128", llvm.target_triple = "x86_64-unknown-linux-gnu", "polygeist.target-cpu" = "x86-64", "polygeist.target-features" = "+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87", "polygeist.tune-cpu" = "generic", spmd.executionKind = "All"} {
  ...
  func.func @main(%arg0: i32, %arg1: memref<?xmemref<?xi8>>) -> i32 attributes {llvm.linkage = #llvm.linkage<external>, spmd.executionKind = "All"} {
    ...
    scf.if %8 {
      ...
      scf.if %11 {
        %33 = func.call @ncclGetUniqueId(%cast) {spmd.executedBy = array<i32: 0>, spmd.executionKind = "Static"} : (memref<?x!llvm.struct<(array<128 x i8>)>>) -> i32
      ...
      scf.for %arg2 = %c0 to %c128 step %c1 {
        %33 = arith.index_cast %arg2 {spmd.executionKind = "All"} : index to i32
        %34 = llvm.getelementptr %20[%33] {spmd.executionKind = "All"} : (!llvm.ptr, i32) -> !llvm.ptr, i8
        %35 = llvm.load %34 {spmd.executionKind = "All"} : !llvm.ptr -> i8
        %36 = llvm.getelementptr %19[%33] {spmd.executionKind = "All"} : (!llvm.ptr, i32) -> !llvm.ptr, i8
        llvm.store %35, %36 {spmd.executionKind = "All"} : i8, !llvm.ptr
      } {spmd.executionKind = "All", spmd.isMultiValued = false}
      ...
      scf.if %23 {
        %33 = memref.load %alloca_1[%c0] {spmd.executionKind = "Dynamic"} : memref<1xmemref<?xi32>>
        %34 = "polygeist.memref2pointer"(%33) {spmd.executionKind = "Dynamic"} : (memref<?xi32>) -> !llvm.ptr
        %35 = "polygeist.pointer2memref"(%34) {spmd.executionKind = "Dynamic"} : (!llvm.ptr) -> memref<?xi8>
        %error_8 = "spmd.allgather"(%5, %35, %c0_i64, %1, %35, %c0_i64, %1, %4) <{isBlocking = true, usedModel = 1 : i32}> {spmd.executionKind = "Dynamic"} : (!spmd.comm, memref<?xi8>, i64, !spmd.datatype, memref<?xi8>, i64, !spmd.datatype, !spmd.stream) -> !spmd.error
      } {spmd.executionKind = "All", spmd.isMultiValued = true}
      %24 = arith.remsi %rank_6, %c2_i32 {spmd.executionKind = "All"} : i32
      %25 = arith.cmpi eq, %24, %c0_i32 {spmd.executionKind = "All"} : i32
      scf.if %25 {
        %33 = memref.load %alloca_1[%c0] {spmd.executionKind = "Dynamic"} : memref<1xmemref<?xi32>>
        %34 = "polygeist.memref2pointer"(%33) {spmd.executionKind = "Dynamic"} : (memref<?xi32>) -> !llvm.ptr
        %35 = "polygeist.pointer2memref"(%34) {spmd.executionKind = "Dynamic"} : (!llvm.ptr) -> memref<?xi8>
      ...
      %26 = memref.load %alloca_0[%c0] {spmd.executionKind = "All"} : memref<1xmemref<?x!llvm.struct<()>>>
      %27 = func.call @cudaStreamSynchronize(%26) {spmd.executionKind = "All"} : (memref<?x!llvm.struct<()>>) -> i32
      %28 = memref.load %alloca_1[%c0] {spmd.executionKind = "All"} : memref<1xmemref<?xi32>>
      ...
      %31 = func.call @cudaFree(%30) {spmd.executionKind = "All"} : (memref<?xi8>) -> i32
      %32 = "spmd.finalize"() <{usedModel = 0 : i32}> {spmd.executionKind = "All"} : () -> !spmd.error
    } {spmd.executionKind = "All", spmd.isMultiValued = false}
